...
export default function PreviewPanel({ loading, generated, template, aiMessage, onRegenHeadline, onRegenCTA }: any) {
  ...
  return (
    <section className="bg-white rounded-3xl shadow-xl p-8 min-h-[700px] overflow-auto">
      ...
      {generated && (
        <>
          {template === 'minimal' && <MinimalTemplate generated={generated} />}
          {template === 'dark' && <DarkTemplate generated={generated} />}
          {template === 'gradient' && <GradientTemplate generated={generated} />}

          <div className="grid grid-cols-2 gap-4 mt-8">
            <button onClick={onRegenHeadline} className="py-3 rounded-2xl bg-slate-200 flex justify-center gap-2"><RefreshCcw size={18}/> Regenerate Headline</button>
            <button onClick={onRegenCTA} className="py-3 rounded-2xl bg-slate-200 flex justify-center gap-2"><RefreshCcw size={18}/> Regenerate CTA</button>
          </div>
        </>
      )}
    </section>
